...
contract Proxy {
    bytes32 private constant IMPLEMENTATION_SLOT = bytes32(uint256(keccak256("eip1967.proxy.implementation")) - 1); // is the contract address you’ll give to users to interact with your contract

    bytes32 private constant ADMIN_SLOT = bytes32(uint256(keccak256("eip1967.proxy.admin")) - 1); // is the contract to manage ownership and upgrades
    ...
    function _setImplementation(address _implementation) private {
        require(_implementation.code.length > 0, "implemention is not contract");
