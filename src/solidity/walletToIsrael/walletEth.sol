...
pragma solidity ^0.8.20;
    ...
    contract Wallet{
        ...
        //Divides an amount equally between all addresses
         function distributeMoney(uint totalAmount) public payable onlyOwner{
            if(msg.sender != owner){
                revert NotAuthorized("Not authorized.");
            }
            if(address(this).balance < totalAmount){
                revert InsufficientFunds("There is not enough money in the contract.");
                //require(msg.value >= totalAmount, "There is not enough money in the contract.");
            }
            uint256 share = totalAmount / wallets.length;
            for (uint i = 0; i < wallets.length; i++) {
                payable(wallets[i]).transfer(share);
            }
        }    
        //Allow the contract to automatically receive payments when they ship to it
        receive() external payable{}

        // Allows the contract owners to withdraw the money in the contract
        function withdraw(uint256 _amount) public onlyOwner{
            if(msg.sender != owner){
                revert NotAuthorized("Only owner can withdraw funds.");
            }
        ...
        //returns the balance of the money in the contract
