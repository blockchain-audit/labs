...
contract StakingRewards {
    ...
    // --- VIEWS
    //מחשב את הפעם האחרונה שהיה שינוי
    //והרווחים מחושבים עד שבעה ימים,
    // לכן - אם כבר עברו שבעה ימים
    //הפונקציה מחזירה את היום השביעי-היום האחרון שהיה רווח
    //ואם עדיין לא נגמרו שבעת הימים הוא מחזיר את התאריך הנוכחי לחישוב מחדש
    function lastTime() public view returns (uint256) {
        return block.timestamp < finish ? block.timestamp : finish;
    }
    //פונקציה זו מחזירה את הערך הצבור ברווח
    //ולכן, אם הבריכה ריקה היא תחזיר את הערך שנצבר עד עכשיו
    //ואם לא- היא מחשבת את הרווח הנוסף
