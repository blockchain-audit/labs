...
contract StakingRewards {
    ...
    function accumulated() public view returns (uint256) {
        ...
        //(rate * (lastTime() - updated) * 1e18) / staked; 
        //כמות הטוקנים לשניה בהתאם לזמן  הנוכחי או הסיום של 7 ימים לחלק לכמות הטוקנים המופקדים 
        //מחזיר את התגמול יחסי לזמן ולכמות הטוקנים בבריכה
        //עבור תגמול בודד 
        return reward + (rate * (lastTime() - updated) * 1e18) / staked;
    }
    //תמיד reward מתעדכן בהתאם לזמן   
    //paid[guy] כל הזמן מתעדכן כי הוא תלוי ב ולכן
    //accumulated שמעדכן את reward
    //וככה הוא מחזיר את התגמול ביחס להפקדה שלו לבנא ספציפי 
    //
    function earned(address guy) public view returns (uint256) {
        //מחזיר תגמול בהתאם לכמות שאני הפקדתי בבריכה 
        return ((balances[guy] * (accumulated() - paid[guy])) / 1e18)
                 + rewards[guy];
    }
    // --- STATE CHANGES
    //        if (guy != address(0)) {
    //בודק עם הכתובת לא null
    ...
    //המנהל הגדיר לעצמו כתובת קבועה יש כזה דבר ?
